...
![npm version](https://img.shields.io/npm/v/@flixora/zeptpay)
![license](https://img.shields.io/npm/l/@flixora/zeptpay)
![downloads](https://img.shields.io/npm/dm/@flixora/zeptpay)

Official ZeptPay SDK for Node.js - Easily integrate merchant onboarding and payment services into your application.

## 📦 Installation

```bash
npm install @flixora/zeptpay
# or
yarn add @flixora/zeptpay
```
...
```env
ZEPTPAY_SECRET_KEY=your_secret_key_here
ZEPTPAY_CLIENT_KEY=your_client_key_here
```
...
const ZEPTPAY_CLIENT_KEY = process.env.ZEPTPAY_CLIENT_KEY || "";
```
- `ZEPTPAY_SECRET_KEY`: Your secret key for authentication (never exposed to frontend)
- `ZEPTPAY_CLIENT_KEY`: Your client key for identification
...
- Includes all important merchant data:
  ...
  - `zeptpayMerchantId`: ZeptPay's system ID
...
### `ZeptPay.flixora.createMerchant(publicKey, merchantData)`

Creates a new merchant in the ZeptPay system.
...
### `ZeptPay.flixora.getMerchantStatus(token)`
...
ZeptPay.events.on('merchantCreated', (data) => {
  console.log('New merchant created:', data);
});

// Listen for status fetches
ZeptPay.events.on('merchantStatusFetched', (data) => {
...
```typescript
import { measureExecutionTime } from '@flixora/zeptpay/performance';
...
```typescript
import { validateMerchantPhone } from '@flixora/zeptpay/rules';
...
```typescript
import { ZeptPayError } from '@flixora/zeptpay/errors';

try {
  await ZeptPay.flixora.createMerchant(publicKey, merchantData);
} catch (error) {
  if (error instanceof ZeptPayError) {
...
- 🐛 Issues: [GitHub Issues](https://github.com/tafseelkhan/zeptpay-sdk-init-server/issues)

## 🚦 Status

![Last Commit](https://github.com/tafseelkhan/zeptpay-sdk-init-server)
