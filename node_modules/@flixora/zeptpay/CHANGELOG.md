...
#### ✨ New Features
- **Merchant Management**
  - `createMerchant` - Create new merchants in ZeptPay system
...
- **Event System**
  ...
  - Easy subscription with `ZeptPay.events.on()`
...
- **Error Handling**
  - Custom `ZeptPayError` class with error codes
...
### 🐛 Bug Fixes
- Fixed type definition for `MerchantResponse` to include `zeptpayMerchantId`
...
### ⚠️ Breaking Changes
- **API Restructuring**
  - Moved all methods under `ZeptPay.flixora` namespace
...
- **CLI Tool**
  - `zeptpay` command-line interface
...
- 🐛 Issues: [GitHub Issues](https://github.com/flixora/zeptpay-sdk/issues)
