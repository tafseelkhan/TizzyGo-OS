...
import sendEmail from "../../../utils/tizzyos/sendEmail";
import sendSMS from "../../../utils/tizzyos/sendSMS";
import generateOTP from "../../../utils/tizzyos/generateOTP";
...
export const checkAuth = async (req: Request, res: Response) => {
  try {
    ...
    if (!decoded.userId) {
      return res
        .status(401)
        .json({ authenticated: false, message: "Unauthorized: userId missing in token" });
    }

    const user = await User.findById(decoded.userId).select("_id name email image");
    if (!user) {
      return res
        .status(401)
